  ...
  return (
    ...
    >
      <div className="flex flex-col leading-none">
        <span className="text-[0.8rem] font-medium uppercase tracking-[0.1em] text-accent">
          {event.month}
        </span>
      ...
      <div>
        ...
        <div className="mt-[1rem] flex flex-wrap gap-x-[1.25rem] gap-y-[0.5rem] text-[0.8rem] font-medium uppercase tracking-[0.06em] text-muted">
          <span>{event.frequency}</span>
          <span>{event.time}</span>
          {event.location && <span>{event.location}</span>}
        </div>
        <AnimatedLink href="#events" arrow className="mt-[1rem] text-[0.85rem] font-medium uppercase tracking-[0.05em] text-ink">
          View Event
        </AnimatedLink>
      </div>
