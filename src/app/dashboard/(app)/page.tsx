  ...
  return (
    <div>
      ...
      <div className="mt-[1.75rem] rounded-[0.375rem] border border-line bg-white p-[1.5rem]">
        ...
        {stats.upcomingEvent ? (
          <div className="mt-[0.625rem] flex flex-col gap-[0.375rem] sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-h3 text-ink">{stats.upcomingEvent.title}</p>
              <p className="text-body mt-[0.25rem] flex flex-wrap items-center gap-x-[1rem] gap-y-[0.25rem] text-muted">
            ...
            <Link
              ...
              className="text-body shrink-0 rounded-[0.4rem] border border-line px-[1.125rem] py-[0.625rem] font-medium text-ink hover:border-subtle"
            >
              View Events
            </Link>
          </div>
        ) : (
          <p className="text-body-lg mt-[0.625rem] text-muted">
      ...
      <div className="mt-[1.5rem] grid grid-cols-1 gap-[1rem] lg:grid-cols-2">
        <div className="rounded-[0.375rem] border border-line bg-white p-[1.5rem]">
          ...
          <div className="mt-[1rem] flex flex-col gap-[0.75rem]">
            {recentMembers.length === 0 ? (
              <p className="text-body rounded-[0.375rem] border border-dashed border-line p-[1.25rem] text-center text-muted">
                No members yet.
              </p>
            ) : (
        ...
        <div className="rounded-[0.375rem] border border-line bg-white p-[1.5rem]">
          <div className="flex items-center justify-between">
            <h2 className="text-h3 text-ink">Upcoming Events</h2>
            <Link href="/dashboard/events" className="text-body font-medium text-ink underline">
              See all
            </Link>
          </div>
          <div className="mt-[1rem] flex flex-col gap-[0.75rem]">
            {upcomingEvents.length === 0 ? (
              ...
              upcomingEvents.slice(0, 4).map((event) => (
                <Link
                  ...
                  className="flex items-center justify-between gap-[0.75rem] rounded-[0.4rem] px-[0.5rem] py-[0.375rem] transition-colors hover:bg-surface"
                >
                  <div className="min-w-0">
                    <p className="text-body truncate font-medium text-ink">{event.title}</p>
                    <p className="text-caption text-muted">
                      {formatEventDate(event.eventDate)}
                      {event.eventTime ? ` · ${event.eventTime}` : ""}
                    </p>
                  </div>
                  <span className="text-caption shrink-0 uppercase tracking-[0.05em] text-accent">
